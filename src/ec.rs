use std::fmt;
use crypto_bigint::{modular::runtime_mod::{DynResidue, DynResidueParams}, Checked};
use crate::Errs;

use super::{Ufeat, Point};
...
impl EllipticCurve {
    pub fn new(name: String, order: Ufeat, modulus: Ufeat, coefficients: [isize; 3]) -> Self {
        if order <= Ufeat::from(2u8) {panic!("Binary fields are a different realm.")}
        ...
        if modulus.wrapping_rem(&Ufeat::from(2u8)) == Ufeat::ZERO {panic!("Fields over even modulus are useless here also modulus can't be zero.")}

        // ~~TODO check that I remember this nuance right; does it called Froebenius track of value 1, btw?~~
            // it's "trace", not "track"
        if order == modulus {println!("This curve could be susceptible to Smart's attack! https://crypto.stackexchange.com/questions/70454/why-smarts-attack-doesnt-work-on-this-ecdlp")}
        ...
        let helper_coef_to_dynres = |coef: isize| -> DynResidue<{Ufeat::LIMBS}> {
            ...
            if !panic_dont {
                ...
                if modulus.as_words().last().unwrap() <= &(coef.unsigned_abs() as u64) {panic!("coefficient is greater or equal to `modulus`")}
            }
            r
        };
        EllipticCurve {
            name, order, 
            a2: helper_coef_to_dynres(coefficients[0]), a4: helper_coef_to_dynres(coefficients[1]), a6: helper_coef_to_dynres(coefficients[2]), 
            // p: NonZero::from_uint(modulus), 
            p, original_p: modulus, 
            type_: if coefficients[0] == 0 {"Weierstrass".to_string()} else {"Montgomery".to_string()},
            original_coefficients: coefficients
        }
    }
    pub fn p(&self) -> &DynResidueParams<{Ufeat::LIMBS}> {&self.p}
    pub fn original_p(&self) -> Ufeat {self.original_p}
    pub fn a2(&self) -> &DynResidue<{Ufeat::LIMBS}> {&self.a2}
    pub fn a4(&self) -> &DynResidue<{Ufeat::LIMBS}> {&self.a4}
    pub fn a6(&self) -> &DynResidue<{Ufeat::LIMBS}> {&self.a6}
    pub fn get(&self) -> (&str, &Ufeat, &[isize], &Ufeat) {(&self.name, &self.order, &self.original_coefficients, &self.original_p)}
    ...
    pub fn discriminant(&self) -> isize {
        ...
        let (b4, b6, b8) = (
            ...
            b2 * self.original_coefficients[2] - self.original_coefficients[1].checked_pow(2).unwrap()
        );
        - b8 * b2.checked_pow(2).expect("discriminant calculation overflow") 
            - 8 * b4.checked_pow(3).expect("discriminant calculation overflow") 
            - 27 * b6.checked_pow(2).expect("discriminant calculation overflow") 
            + 9 * b2 * b4 * b6
    }

    ///        Tests if the elliptic curve is smooth or not
    pub fn is_smooth(&self) -> Result<bool, Errs> {
        if self.discriminant() == 0 {Err(Errs::Exception)}
        else {Ok(true)}
    }
    ...
    pub fn contains(&self, point: &Point) -> bool {
        ...
        let rhs = 
            x_modp.pow(&Ufeat::from(3u8))
            + self.a2 * x_modp.square()
            + self.a4 * x_modp
            + self.a6;
        // debug print
    ...
    fn default() -> Self {
        ...
        EllipticCurve::new(
            ...
            (
                ...
                - Checked::new(Ufeat::from_be_hex(&("0".repeat(Ufeat::BITS / 4 - 34) + "014551231950b75fc4402da1732fc9bebf"))) 
            ).0.unwrap(), 
            (
                Checked::new(two_pow256)
...
-------------------------
    ...
    equation: y^2 = x^3 + a2.x^2 + a4.x + a6 (mod p)",
            self.name, self.order, self.original_coefficients[0], self.original_coefficients[1], self.original_coefficients[2], self.original_p
        )
    }
}
